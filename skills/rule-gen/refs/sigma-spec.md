...
tags:
    ...
    - attack.t1059.001          # sub-technique (procedure-level) — technique/sub-technique tags ONLY
logsource:
    category: <category>
...
Tags use the `attack.` prefix for MITRE ATT&CK mapping. **Actioner convention: technique/sub-technique (procedure-level) tags ONLY — never tactic tags.** Tactic tags (`attack.execution`, `attack.defense_evasion`, …) carry no detection value and several trip the pySigma `InvalidATTACKTagIssue` validator, so omit them. Capture tactic context in the report's `## MITRE ATT&CK Mapping` table instead, not in rule tags.

- **Techniques**: `attack.t1059` (Command and Scripting Interpreter)
- **Sub-techniques**: `attack.t1059.001` (PowerShell) — prefer the most specific sub-technique with clear evidence
