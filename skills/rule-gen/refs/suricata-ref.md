...
|---|---|---|
| `msg` | Alert message (required) | `msg:"Actioner - Cobalt Strike C2 Beacon";` |
...
| `priority` | Override priority (1–255; 1–4 commonly used) | `priority:1;` |
| `metadata` | Key-value metadata | `metadata:author Actioner, created_at 2026-04-01;` |
...
`tls.version` is **not** a sticky buffer — it is a value keyword: `tls.version:1.2;` (valid values `1.0`, `1.1`, `1.2`, `1.3`, or hex like `0x0303`). Do not follow it with `content`.
...
alert tls $HOME_NET any -> $EXTERNAL_NET any (
    ...
    metadata:author Actioner, created_at 2026-04-01;
    sid:2100001;
    rev:1;
)
```
...
alert dns $HOME_NET any -> any any (
    msg:"Actioner - DNS Query to Known C2 Domain";
    flow:to_server;
    dns.query;
...
alert http $HOME_NET any -> $EXTERNAL_NET any (
    msg:"Actioner - HTTP POST Exfiltration with Suspicious UA";
    flow:established,to_server;
    http.method;
    content:"POST";
    http.user_agent;
...
```
alert tls $HOME_NET any -> $EXTERNAL_NET any (
    msg:"Actioner - TLS Connection to Known Malicious Certificate";
    flow:established,to_server;
...
### Example 5: Repeated Malformed SSH Banner

`app-layer-event:ssh.invalid_banner` fires on malformed/invalid SSH banners (a protocol anomaly), not on repeated logins — do not label it brute force.

```
alert ssh any any -> $HOME_NET 22 (
    msg:"Actioner - Repeated Invalid SSH Banner from Source";
    flow:to_server;
...
| Buffer size match | `bufferlen` | `bsize` |
| Config test command | `snort -c snort.lua -R rules.rules -T` | `suricata -T -S rules.rules -l /tmp` |
