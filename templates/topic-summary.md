...
<!-- PLACEHOLDER: the Rule Generator replaces this line with Sigma/YARA/Snort/Suricata detection rules. Open with a 2–3 sentence summary (what the rules cover + the one caveat that matters). Per rule: ONE sentence + status line (compile ✅/⚠️ × confidence high/medium/low) + the fenced rule; ≤1 load-bearing caveat; put all validation/encoding/provenance/FP-evasion detail in a single HTML audit comment, not reader prose. If the source has no concrete artifacts, state "No production-ready detection" here instead of inventing a broad rule. -->
